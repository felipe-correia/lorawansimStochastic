...
input_ED_dc_limit_percent=100;%duty cycle limit for ED UL
input_RX1_dc_limit_percent=100;%duty cycle limit for GW RX1 DL
input_RX2_dc_limit_percent=100;%duty cycle limit for GW RX2 DL
...
input_func_UL_max_distance=@km_LoRaWAN_model_max_propagation_distance;%determines max distance reachable in UL for each DR, in meters 
input_func_UL_propagation_model=@km_LoRaWAN_model_propagation_loss;%propagation of UL signals [debugging:not checked]
input_func_DL_propagation_model=@km_LoRaWAN_model_propagation_loss;%propagation of DL signals [debugging:not checked]
input_func_GW_to_GW_propagation_model=@km_LoRaWAN_model_propagation_loss;%propagation of signals between GWs (interference)
input_func_ED_to_ED_propagation_model=@km_LoRaWAN_model_propagation_loss;%propagation of signals between EDs (interference) 
...
%remove DL interference     temp_DL_RX1_packet_flag_received=true;
                    ...
                    if input_energy_simulate==true
                        ...
                        for temp_ED_number=1:1:input_num_EDs
                            ...
                            %packets lost
                            lost_packets=temp_num_UL_transmissions_delivered-(temp_num_DL_received_in_RX1+temp_num_DL_received_in_RX2); 
                            temp_ED_idle_duration_ms=temp_num_DL_received_in_RX1*input_RX1_delay_s*1000+(temp_num_DL_received_in_RX2+lost_packets)*(input_RX1_delay_s+input_RX2_delay_s)*1000;
